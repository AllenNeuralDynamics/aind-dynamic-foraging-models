...
from .fitting_functions import cross_validate_bandit, fit_bandit
...
class BanditModelComparison:
    ...
    def __init__(
        self, choice_history, reward_history, model, iti=None, p_reward=None, session_num=None
    ):
        """
        ...
        self.models = model
        self.fit_choice_history, self.fit_reward_history, self.p_reward, self.session_num = (
            choice_history,
            reward_history,
            p_reward,
            session_num,
        ...
        assert (
            np.shape(self.fit_choice_history)[1] == self.n_trials
        ), "Choice length should be equal to reward length!"

    def fit(
        self,
        fit_method="DE",
        fit_settings={"DE_pop_size": 16},
        pool="",
        if_verbose=True,
        ...
        for mm, model in enumerate(self.models):
            ...
            if if_verbose:
                print(
                    "Model %g/%g: %15s, Km = %g ..." % (mm + 1, len(self.models), forager, Km),
                    end="",
                )
            start = time.time()

            result_this = fit_bandit(
                forager,
                fit_names,
                fit_bounds,
                self.fit_choice_history,
                self.fit_reward_history,
                iti=self.fit_iti,
                ...
                **fit_settings,
                pool=pool,
                if_predictive=True,
            )  # plot_predictive is not None)

            if if_verbose:
                print(
                    " AIC = %g, BIC = %g (done in %.3g secs)"
                    % (result_this.AIC, result_this.BIC, time.time() - start)
                )
                ...
                pd.DataFrame(
                    {
                        ...
                        "LPT": result_this.LPT,
                        "prediction_accuracy": result_this.prediction_accuracy,
        ...
        if (
            plot_predictive is not None
        ):  # Plot the predictive choice trace of the best fitting of the best model (Using AIC)
            self.plot_predictive = plot_predictive
            self.plot_predictive_choice()
        return

    def cross_validate(
        self, k_fold=2, fit_method="DE", fit_settings={"DE_pop_size": 16}, pool="", if_verbose=True
    ):
        ...
        for mm, model in enumerate(self.models):
            ...
            if if_verbose:
                print(
                    "  \n%g-fold CV: Test acc.= %s, Fit acc. = %s (done in %.3g secs)"
                    % (
                        k_fold,
                        prediction_accuracy_test,
                        prediction_accuracy_fit,
            ...
            self.prediction_accuracy_CV = pd.concat(
                [
                    self.prediction_accuracy_CV,
                    pd.DataFrame(
                        {
                            "model#": mm,
                            "forager": forager,
