...
from .util import choose_ps, moving_average, softmax
...
def step_RWlike_ignore(
    choice,
    reward,
    valid_reward_history,
    q_estimation_tminus1,
    forget_rates,
    learn_rates,
    ignore_rates,
):
...
def step_ANCCR(
    choice, reward, q_estimation_tminus1, learn_rate
):  # , et, lam, base_rate,  m, mbs, mbsa, w):
    # et *= lam
    # et[0, choice] += 1.
    # if np.random.rand() < base_rate:
    # mbsa += learn_rate * (et - mbsa)
    ...
    q_estimation_t = np.zeros_like(q_estimation_tminus1)
    K = q_estimation_tminus1.shape[0]
    q_estimation_t[choice] = (1 - forget_rate) * q_estimation_tminus1[choice] + learn_rate * (
        reward - q_estimation_tminus1[choice]
    )
    # Unchosen: Q(n+1) = (1-forget_rate_unchosen) * Q(n)
    unchosen_idx = [cc for cc in range(K) if cc != choice]
...
def act_Probabilistic_ignore(
    q_estimation_t=0,
    softmax_temperature=0,
    ignore_softmax_temperature=0,
    bias_terms=0,
    ignore_bias=1.0,
    choice_softmax_temperature=None,
    choice_kernel=None,
    ...
    if choice_kernel is not None:
        ...
        ignore_sotfmax_temperature = np.array(
            [ignore_softmax_temperature, choice_softmax_temperature]
        )[np.newaxis, :]
    choice_prob = np.zeros(q_estimation_t.shape[:1])
    choice_prob[-1] = sigmoid(
        np.sum((q_estimation_t[-1] - ignore_bias) / ignore_softmax_temperature)
    )
    choice_prob[:-1] = (1.0 - choice_prob[-1]) * softmax(
        q_estimation_t[:-1], temperature=softmax_temperature
    )
    choice = choose_ps(choice_prob)
    return choice, choice_prob
...
class ForagerModel:
    ...
    def fit_each_init(
        ...
        fitting_result = optimize.minimize(
            negLL_func,
            x0,
            args=(banditmodel, fit_names, fit_choice_history, fit_reward_history, {}),
            method=fit_method,
            bounds=optimize.Bounds(fit_bounds[0], fit_bounds[1]),
        )
        return fitting_result
...
class forager_Hattori2019(ForagerModel):
    ...
    def act(
        ...
        act_kwargs = {
            "q_estimation_t": self.q_estimation[:, self.time],
    ...
    def fit_history(self, fit_choice_history, fit_reward_history, pool=""):
        fitting_result = super().fit_history(
            self.banditmodel,
            fit_choice_history,
            fit_reward_history,
            self.fit_names,
        ...
        **kwargs,
        ...
        self.fit_names.extend(
            [
                ...
                "ignore_rate_satiety",
                "ignore_rate_Qs",
                "ignore_softmax_temperature",
                "biasL",
                "biasR",
            ]
        )
        self.fit_std_values.extend([0.5, 0.1, 0.2, 0.3, 0.01, 0.4, 12.0, 0.3, 0.3])
