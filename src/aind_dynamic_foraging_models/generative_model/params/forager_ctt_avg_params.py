"""Generate pydantic models for ctt_dualQ foraging agent parameters."""
...
def generate_pydantic_ctt_avg_params(
    ...
    # Forget rate for value update
    params_fields["forget_rate_unchosen"] = (
        float,
        Field(default=0.5, ge=0.0, le=1.0, description="Forget rate for value update"),
    )
    fitting_bounds["forget_rate_unchosen"] = (0.0, 1.0)

    # threshold_offset (ρ) - the value to compare against
    params_fields["threshold_offset"] = (
        float,
        Field(default=0.4, description="Threshold value for comparison (ρ)"),
    )
    fitting_bounds["threshold_offset"] = (-1.0, 1.0)
