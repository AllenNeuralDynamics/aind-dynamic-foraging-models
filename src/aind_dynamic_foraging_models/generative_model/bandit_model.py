...
from .util import choose_ps, softmax
...
class BanditModel:
    ...
    def __init__(
        ...
        if "LNP_softmax" in forager:
            ...
            assert all(x is not None for x in (learn_rate, tau_cann, softmax_temperature))
            self.tau_cann = tau_cann
            self.learn_rates = [learn_rate, learn_rate]

        elif "Synaptic" in forager:
            assert all(
                x is not None for x in (learn_rate, forget_rate, I0, rho, softmax_temperature)
            )
            self.I0 = I0
            self.rho = rho
            self.learn_rates = [learn_rate, learn_rate]
    ...
    def reset(self):
        ...
        if self.if_fit_mode:  # Predictive mode
            ...
            self.reward_available[:, 0] = (
                np.random.uniform(0, 1, self.K) < self.p_reward[:, self.time]
            ).astype(int)
        ...
        if self.forager in [
            ...
            for tau, w_tau in zip(self.taus, self.w_taus):
                ...
                self.history_filter += (
                    w_tau * np.exp(-reversed_t / tau) / np.sum(np.exp(-reversed_t / tau))
                )
    ...
    def generate_p_reward(
        self,
        block_size_base=global_block_size_mean,
        block_size_sd=global_block_size_sd,
        # (Bari-Cohen 2019)
        p_reward_pairs=[[0.4, 0.05], [0.3857, 0.0643], [0.3375, 0.1125], [0.225, 0.225]],
    ):
        ...
        while n_trials_now < n_trials:
            ...
            n_trials_this_block = np.rint(np.random.normal(block_size_base, block_size_sd)).astype(
                int
            )
    ...
    def get_AmBn_choice_history(self, p_reward_this_block, n_trials_this_block, n_trials_now):
        ...
        c_max_this = np.argwhere(p_reward_this_block[0] == np.max(p_reward_this_block))[
            0
        ]  # To handle the case of p0 = p1
        c_min_this = np.argwhere(p_reward_this_block[0] == np.min(p_reward_this_block))[-1]
        # Choice pattern of {m_star, 1}
        c_star_this_block = ([c_max_this] * mn_star[0] + [c_min_this] * mn_star[1]) * S
        # Truncate to the correct length
        c_star_this_block = c_star_this_block[:n_trials_this_block]

        self.choice_history[0, n_trials_now : n_trials_now + n_trials_this_block] = (
            c_star_this_block  # Save the optimal sequence
        )
    ...
    def act_LossCounting(self):
        ...
        if self.if_fit_mode:
            ...
            prob_switch = norm.cdf(
                self.loss_count[0, self.time],
                self.loss_count_threshold_mean - 1e-6,
                self.loss_count_threshold_std + 1e-16,
            )
    ...
    def act_EpsiGreedy(self):
        ...
        choice = np.random.choice(
            np.where(self.q_estimation[:, self.time] == self.q_estimation[:, self.time].max())[0]
        )
        ...
        if self.if_fit_mode:
            ...
            self.predictive_choice_prob[choice, self.time] = (
                1 - self.epsilon + self.epsilon * (1 / self.K + self.bias_terms[choice])
            )
            choice = None  # No need to make specific choice in fitting mode
        else:
            if np.random.rand() < self.epsilon:
    ...
    def act_Probabilistic(self):
        ...
        if "_CK" in self.forager:
            self.choice_prob[:, self.time] = softmax(
                np.vstack([self.q_estimation[:, self.time], self.choice_kernel[:, self.time]]),
                np.vstack([self.softmax_temperature, self.choice_softmax_temperature]),
                bias=self.bias_terms,
            )  # Updated softmax function that accepts two elements
        else:
            self.choice_prob[:, self.time] = softmax(
    ...
    def step_LossCounting(self, reward):
        ...
        if self.loss_count[0, self.time - 1] < 0:  # A switch just happened
            ...
            self.loss_count[0, self.time - 1] = -self.loss_count[0, self.time - 1]
            if reward:
    ...
    def step_RWlike(self, choice, reward):
        ...
        self.q_estimation[choice, self.time] = (1 - self.forget_rates[1]) * self.q_estimation[
            choice, self.time - 1
        ...
        self.q_estimation[unchosen_idx, self.time] = (1 - self.forget_rates[0]) * self.q_estimation[
            unchosen_idx, self.time - 1
        ]
    ...
    def step_CANN(self, choice, reward):
        ...
        # Choice-dependent forgetting rate ('Hattori2019')
        # Chosen:   Q(n+1) = (1- forget_rate_chosen) * Q(n) + step_size * (Reward - Q(n))
        self.q_estimation[choice, self.time] = (
            self.q_estimation[choice, self.time - 1]
            + learn_rate_this * (reward - self.q_estimation[choice, self.time - 1])
        ) * np.exp(-iti_time_minus1_to_time / self.tau_cann)

        # Unchosen: Q(n+1) = (1-forget_rate_unchosen) * Q(n)
        unchosen_idx = [cc for cc in range(self.K) if cc != choice]
        self.q_estimation[unchosen_idx, self.time] = self.q_estimation[
            unchosen_idx, self.time - 1
        ] * np.exp(-iti_time_minus1_to_time / self.tau_cann)
    ...
    def step_synaptic(self, choice, reward):
        ...
        # Chosen side
        self.w[choice, self.time] = (1 - self.forget_rates[1]) * self.w[
            choice, self.time - 1
        ] + learn_rate_this * (
        ...
        # Unchosen side
        self.w[1 - choice, self.time] = (1 - self.forget_rates[0]) * self.w[
            1 - choice, self.time - 1
        ]
        ...
        # -- Update u --
        for side in [0, 1]:
            self.q_estimation[side, self.time] = self.f(
                self.I0
                * (1 - self.w[1 - side, self.time])
                / (
                    self.w[:, self.time].prod()
                    - (1 + self.rho / 2) * self.w[:, self.time].sum()
                    + 1
                    + self.rho
                )
            )
    ...
    def step_choice_kernel(self, choice):
        ...
        self.choice_kernel[:, self.time] = self.choice_kernel[
            :, self.time - 1
    ...
    def act(
        ...
        if self.forager in [
            ...
            "Synaptic",
            "Synaptic_W>0",
        ]:  # Probabilistic (Could have choice kernel)
    ...
    def step(
        ...
        if not self.if_fit_mode:
            ...
            self.reward_available[:, self.time] = np.logical_or(
                reward_available_after_choice * self.if_baited,
                np.random.uniform(0, 1, self.K) < self.p_reward[:, self.time],
            ).astype(int)
        ...
        if (
            "_CK" in self.forager
        ):  # Could be independent of other foragers, so use "if" rather than "elif"
