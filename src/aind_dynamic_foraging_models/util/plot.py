...
def plot_session_lightweight(
    ...
    unrewarded_trials = (
        (~ignored_trials) & (~rewarded_trials_non_autowater) & (~rewarded_trials_autowater)
    )
    ...
    if photostim is not None:
        ...
        scatter = ax_1.scatter(
            *(x, y) if not vertical else [*(y, x)],
            ...
            edgecolors=[plot_spec_photostim[t] for t in s_type] if any(s_type) else "darkcyan",
    ...
    ll = p_reward[0, :]
    rr = p_reward[1, :]
    ax_2.plot(*(xx, rr) if not vertical else [*(rr, xx)], color="b", label="p_right", lw=1)
    ax_2.plot(*(xx, ll) if not vertical else [*(ll, xx)], color="r", label="p_left", lw=1)
