...
from .params.forager_ctt_dualQ_params import generate_pydantic_ctt_dualQ_params


class ForagerCTTDualQ(DynamicForagingAgentMLEBase):
    ...
    def _get_params_model(self, agent_kwargs):
        ...
        """
        return generate_pydantic_ctt_dualQ_params(**agent_kwargs)
    ...
    def act(self, _):  # noqa: C901
        ...
        if self.trial == 0:
            p_exploit = 1 / (1 + np.exp(-beta * (q_value[0] - threshold)))
        
        else:
            # a_{t-1} is L
            if prev_choice == 0:
                p_exploit = 1 / (1 + np.exp(-beta * (q_value[0] - threshold) - self.params.biasL))
            # a_{t-1} is R
            elif prev_choice == 1:
                p_exploit = 1 / (1 + np.exp(-beta * (q_value[1] - threshold)))
            else:
    ...
    def learn(self, _observation, choice, reward, _next_observation, done):
        ...
        # unchosen action: forget
        self.q_value[1 - choice, self.trial] = (1 - self.params.forget_rate_unchosen) * self.q_value[1 - choice, self.trial - 1]
